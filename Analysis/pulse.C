...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries; jentry++) {
      ...
      for (int i = 0; i < nhits[0]; i++) {
          Nhits_all+=1;
          if (tot_code[i] == 480) continue; // REMOVING THE UNEXPLAINED TOT VALUES
          double toa = toa_code[i] * 3.125 / cal_code[i];
          double tot = ( 2 * tot_code[i] - floor(tot_code[i] / 32) ) * (3.125 / cal_code[i]);
          tot_mean_all+=tot;
          if (tot < tot_min_all) tot_min_all = tot;
          if (tot > tot_max_all) tot_max_all = tot;
          TOT_all->Fill(tot);
          TOA_all->Fill(toa);
          CAL_all->Fill(cal_code[i]);
          TOA_TOT_all->Fill(toa, tot);
          HEATMAP_all->Fill(col[i], row[i]);
          TOT_PIXEL_all->Fill((row[i]*16+col[i]), tot);
          // if (cal_code[i] == cal_values[row[i]][col[i]] && toa > 9 && toa < 10) { // Element level selections              Nhits+=1;
          if (cal_code[i] > 182 && cal_code[i] < 190 && tot > 2.5 && nhits[0] > 0) { // Element level selections
              bool skip_event = true;
              for (int b = 0; b < sizeof(pixel_map)/sizeof(pixel_map[0]); b++) {
                  if (std::round(row[i]*16+col[i])==(pixel_map[b]-1)) {
                      skip_event = false;
                  }
              }
              ...
              HEATMAP->Fill(col[i], row[i]); // GetIndex(std::round(row[i]*16+col[i]))
              TOT_PIXEL->Fill(GetIndex(std::round(row[i]*16+col[i])), tot); // (row[i]*16+col[i]), tot);
              TOT_sel_pixels[GetIndex(std::round(row[i]*16+col[i]))] -> Fill(tot);
